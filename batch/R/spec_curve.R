...
plot_spec_curve <- function(
  ...
  # Order wx_ref_period so facets render 1 month at top (ggplot2 draws levels bottom-to-top)
  ref_period_levels <- data |>
    dplyr::distinct(.wx_n, wx_ref_period) |>
    dplyr::arrange(dplyr::desc(.wx_n)) |>
    dplyr::pull(wx_ref_period)
  data <- data |>
    dplyr::mutate(wx_ref_period = factor(wx_ref_period, levels = ref_period_levels)) |>
    dplyr::select(-.wx_n)
  ...
  grid_long <- spec_df |>
    ...
    dplyr::mutate(
      # Restore factor levels lost by pivot_longer so y-axis order is correct
      spec_value = {
        ordered_lvls <- unique(unlist(lapply(avail_in_spec, function(v) {
          col <- data[[v]]
          if (is.factor(col)) as.character(levels(col)) else as.character(unique(col[!is.na(col)]))
        })))
        factor(as.character(spec_value), levels = ordered_lvls)
      },
      spec_var = factor(
        spec_var,
