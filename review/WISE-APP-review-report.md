...
- **Regenerating the manifest:** whenever dependencies or the shipped file set change, run in `dev/03_deploy.R`: `rsconnect::writeManifest(appDir = ".", appFiles = c("app.R", "R", "inst", "man", "DESCRIPTION", "NAMESPACE"), appPrimaryDoc = "app.R")`, then **re-strip `sf`, `terra` and `geojsonsf`** from `manifest.json$packages` before committing — the Connect host cannot build `sf` (GDAL/GEOS/PROJ), and since the 2026-09-02 MapLibre basemap migration `mapgl` lists `sf`/`terra`/`geojsonsf` in Imports too. The strip is safe because neither `leaflet` nor `mapgl` loads them at runtime: mapgl's NAMESPACE only `importFrom(classInt, classIntervals)` (pure R, kept in the manifest), and the app's executed paths (`add_source` with GeoJSON strings, `fit_bounds` with numeric bbox) never touch sf. Verify with `grep -c '"sf":' manifest.json` → `0`, then commit the diff.
