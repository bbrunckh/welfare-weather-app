...
diag_content_ui <- function(ns, so, weather_var_choices, scenario_names = character(0)) {
  ...
  tagList(
    ...
    # Rendered server-side so new scenarios saved after tab-open appear here.
    ...
    shiny::wellPanel(
      ...
      shiny::tags$p(
        ...
        " weather values at locations and months in the survey (interview years only).",
        shiny::tags$br(),
        shiny::tags$b("Grey = Full historical:"),
        " same locations and months, all years in the simulation window.",
        shiny::tags$br(),
        shiny::tags$b("Coloured lines = Future scenarios:"),
        " perturbed weather; solid = earliest year, dashed = middle, dotted = latest.",
        shiny::tags$br(),
        "Raw view: one unique value per location x month x year.",
        " Values sourced from pre-transformation weather_raw."
      )
    ),
    ...
    shiny::wellPanel(
      ...
      # Dynamic height plotOutput -- rebuilt when primary group or spacing changes
      ...
      shiny::tags$p(
        ...
        " one row per scenario; grey-scale lines = individual historical years",
        " (darkest = most recent).",
        shiny::tags$br(),
        shiny::tags$b("Forecast year × Scenario mode:"),
        " same as Scenario mode but rows sorted by forecast year, then SSP.",
        shiny::tags$br(),
...
mod_2_05_sim_diag_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(selected_weather(), {
      req(diag_tab_added())
      sw      <- selected_weather()
      choices <- if (!is.null(sw) && "name" %in% names(sw)) sw$name else character(0)
      current <- isolate(input$diag_weather_vars)
      new_sel <- if (!is.null(current)) intersect(current, choices) else choices[seq_len(min(2, length(choices)))]
    ...
    output$scenario_filter_panel <- shiny::renderUI({
      ...
      if (length(unique_ssps) == 0 && length(unique_yrs) == 0) return(NULL)
    ...
    active_scenarios_data <- reactive({
      ...
      # Read the two inline checkboxGroupInput vectors.
      # NULL means the UI hasn't rendered yet -- treat as all selected.
    ...
    ridge_kde_data <- reactive({
      ...
      # log_scale is baked into kde_data so BW is correct for current setting;
      # note: if user toggles log after KDE is cached the BW will be slightly
      # off -- acceptable trade-off vs re-running 270 density() calls per tick.
    ...
    output$diag_weather_density <- renderPlot({
      ...
      plot_weather_density_panel(
        ...
        log_x            = isTRUE(input$diag_weather_log_x)
      )
    })
