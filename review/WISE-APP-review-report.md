...
> **Remediation status (through 2026-09-01):** Five fix waves applied; completed rows are compressed under their sections, open rows keep full detail. Wave 0 determinism (DET-01..09) complete 2026-08-31, plus two low-risk batches and scoped parts of REACT-01/13, TEST-03/05, RED-09. On 2026-09-01: a blocker batch (DEP-01, INT-02/09, DEP-02/03 incl. the REACT-01 remainder, REACT-05, SEC-02, TEST-06/09 gradient bugs), a test batch (TEST-07 raw-weights contract, TEST-08 packaging), four output-neutral, equivalence-verified perf batches — **Batch A** (PERF-23b/08/25/10), **Batch B** (DUP-01, PERF-30/REACT-07), **Batch C** (PERF-22/34, RNG-stream-invariant, bit-identical), **Batch D** (PERF-32) — a quick-win batch (INT-04, INT-07, UI-04, UI-38, RED-05; REACT-04/06 closed as no-action), three High-severity fixes (REACT-12 failure ledger + TEST-01 remainder; REACT-02 busy guards + UI-01 visible errors; PERF-31 Step 3 aggregation cache), the remaining standalone perf wins PERF-23 (Step 1 h3 temp table), PERF-13 (versioned weather disk cache), PERF-16 (dev mode removed entirely), and the Wave 0.5 completion batch (INT-01 selection restore, INT-06 map/cell state clearing, INT-05 fit-time snapshots, INT-08 run signatures + stale-marking — closing all remaining code items in Wave 0.5; only the DEP-01/DEP-02 live-backend verifications remain), a Step 1 responsiveness batch (Outcome stats and Weather stats tabs now render from button-time selection snapshots — selector changes no longer re-render or re-aggregate until the button is re-pressed, matching the sample/model tabs; survey-reload staleness banners were added to both tabs, the only staleness source that applies to them), and a correctness fix surfaced by the INT-04 name restoration (the per-model RP matrix was transposed when only the median RP is admissible — two simulation years with several ensemble members — duplicating threshold-table rows; both Step 2 and Step 3 builders now share a shape-safe helper). Full edition-3 suite: **639/639 pass**, no warnings; built-source `R CMD check` clean at the code level (remaining findings environment-only — no `pdflatex`); `R CMD build` succeeds. Deployment commits `0b73a4c`/`459d3f7`/`fdd3960` — see §10.1.
...
**Done, compressed:** INT-01 (2026-09-01 — shared `.restore_selection()`/`.restore_numeric()` helpers in `utils_ui.R`; every dynamic input now restores the previous selection clipped to the new choice set and falls back to the historical defaults only when nothing survives: Step 1 year selectors, weather selector, model-panel interactions/fixed effects/covariate selects/Lasso force + advanced panel, Step 2 baseline survey, Step 3 poverty line and scenario filters; Step 3 restore verified via testServer HTML rendering) · INT-08 (2026-09-01 — stale-mark semantics per user decision: immutable run signatures (`.sig`, plain-list canonicalisation via `.sig_plain()`) stored on every result at creation — Step 1 fit (survey version + shape + outcome + weather + model spec), Step 2 simulation (fit signature + hist years + SSPs + future periods + baseline survey + residuals + uncertainty flags), Step 3 policy (sim signature + survey version + all five scenario configs, captured at run start); module-level staleness observers compare live inputs against the stored signature and mark results stale, with the Step-2 flag cascading into Step 3; shared `.stale_banner()` warns above the Step 1 Results and Model-fit tabs, Step 2 Results and Diagnostics tabs, and the Step 3 results pane; the only export surfaces (the two DT CSV buttons in mod_2_02 and the Step 3 pane) drop their CSV button while stale; full app-server wiring smoke-tested via testServer) · INT-05 (2026-09-01 — every fit result now carries a fit-time snapshot (`.snap`: outcome row, weather selection, survey-weather frame, variable-label frame) and all Step 1 renderers bind to it — coefficient/effect/regression tables in mod_1_07 and the diagnostic plots in mod_1_08 describe the fitted run, not the live selections; engine-conditional headings converted to reactive outputs so a re-fit with a different engine updates the wording; Step 3's historical label is captured into the sim result (`hist_label`) at publish and preferred over the live selection, with the live selection only as fallback for older in-memory objects; testServer-tested) · INT-06 (2026-09-01 — the survey-load observer clears `map_data()`/`cell_data()` as soon as a reload starts, so an H3/map/cell/panel failure leaves a blank map instead of the previous survey's geography, and the `loc_id_panel` failure warning now states that variance estimation falls back and inference changes; testServer-tested with a mocked `load_data`) · INT-02 (2026-09-01 — manual connect resets metadata up front, loads each file into a failure ledger, publishes the connection only when every load succeeds, persistent failure card; auto-connect shares the channel) · INT-03 (2026-08-31 — aggregators prefer the run-time residual snapshot over the live radio) · INT-04 (2026-09-01 — Step 3 scenario-aggregation failures collected and surfaced as a persistent warning toast naming the dropped scenarios, deduped per failure set) · INT-07 (2026-09-01 — Results/Diagnostics tabs track the `hist_sim` lifecycle: appended on run, `removeTab()` + flag reset on clear, re-appended on re-run; testServer-tested) · INT-09 (2026-09-01 — Step 3 computes in locals and publishes reactives + `sim_run_id` only after simulation and decomposition both succeed).
...
**Done (2026-09-01), compressed (Batches A-D + PERF-31/23/13, all output-neutral, equivalence/bit-identical verified):** PERF-08 (single grouped `rowsum()` completeness pass, ~3x at 200k×72) · PERF-10 (parsed GeoJSON copies dropped from feature builders; `.geojson_bounds()` parses string-only features transiently) · PERF-13 (`fct_get_weather.R` bounded disk cache: every remote parquet load goes through `.wx_cache_load()` — slice (columns + date range) COPY'd once to a local parquet keyed by digest(cache version, files, columns, date range); cache hit loads locally without opening the remote store; CMIP6 historical file opened once and shared by the resolution probe *and* the monthly baseline; each SSP future file fetched once and reused for the baseline overlap + every future period; versioned key (`WISEAPP_WX_CACHE_VERSION`), size-capped (default 2 GB, LRU by mtime), `WISEAPP_WEATHER_CACHE_DISABLE` kill switch, `WISEAPP_WEATHER_CACHE_FORCE` test override, graceful fallback to remote on write failure; DuckDB single-thread guarantees scan order → bit-identical, verified end-to-end cold+warm vs cache-disabled climate run) · PERF-22 (policy deltas + training ecdf precomputed once per run and threaded through the per-(scenario × year) decomposition loop; 1.5x at 50k HH, RNG-free, bit-identical) · PERF-23 (Step 1 survey-stats block materialises the remote `h3_df` view into a local DuckDB temp table once (`dplyr::compute(temporary = TRUE)`), so the ~6 downstream scans (map GeoJSON, cell geometry, cell map, loc_panel passes, loc keys) read locally; dropped on handler exit; `loc_panel` verified identical on the view vs the temp table) · PERF-23b (hash-cached `CREATE OR REPLACE SECRET` extended to S3/GCS/Azure) · PERF-25 (grouping hoisted out of the per-variable weather loop via `.summarise_loc_prep(prep =)`) · PERF-30 (display-only `band_q` dropped from `agg_workspace()`; poverty-line/bandwidth folded into method-specific cache keys, so moving those controls no longer destroys unrelated cached methods) · PERF-31 (Step 3 aggregation cache in `fct_policy_sim_compare.R`: fresh cache environment on any sim republish, keyed by (source, method, poverty line) only — `cmp_deviation` is applied downstream (hist_ref subtraction + axis labels via the new `agg_axis_label()`), so moving the deviation control no longer re-aggregates baseline + every scenario member; `.wire_results_pane()` exposes internals for tests; cached ≡ uncached verified bit-identically across the method/deviation/pov-line grid, testServer invalidation matrix) · PERF-32 (prediction frame released before the N×K matrix build; process peak now bounded by the join phase, byte-identical) · PERF-34 (ID→residual lookup + residual variance built once per pipeline/member; bit-identical, RNG unchanged) · PERF-16 (2026-09-01 — `dev_mode` removed entirely per user decision: UI checkbox, `fct_run_simulation()` param + key-dedup block, `DEV_MODE` in all 7 batch scripts, and the `.claude/` doc mentions; the "All models" provenance text is now simply always true). Regression coverage: `test-mod_2_02_results.R`, `test-policy-sim-compare-agg-cache.R`, `test-fct_wx_cache.R`, `test-fct_get_weather.R`.
...
**Done (2026-09-01), compressed:** UI-01 (output errors are now visible as warning cards instead of hidden — `custom.css` blanket hide removed, validation messages stay muted-italic; audited all long renderers: every one either `req()`-guards (validation-class, stays silent) or returns NULL gracefully on the pre-run path, so no raw-error leakage) · UI-04 (colorblind-safe palettes: Okabe-Ito centralized in `utils_plot_theme.R` with `wise_scale_colour/fill_okabe_ito()` replacing ColorBrewer Set1 in all result/decomposition plots; SSP colours now bluish green/blue/vermillion; coverage map ramp now vermillion/orange/bluish green) · UI-38 (unchecking the final Step 2 scenario checkbox re-checks the held scenarios instead of silently re-displaying the first one; the grid can never drop its last selection).
