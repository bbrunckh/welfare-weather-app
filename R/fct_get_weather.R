...
#' Summarise a CMIP6 ensemble to mean + lower/upper representative series.
#'
#' Replaces the per-model entries in a `get_weather()` result list with
#' three representative weather series per SSP/period group:
#' \itemize{
#'   \item \code{<ssp>_<period>_ensemble_mean} — mean across models
#'   \item \code{<ssp>_<period>_ensemble_lo}   — lower quantile (default 10th)
#'   \item \code{<ssp>_<period>_ensemble_hi}   — upper quantile (default 90th)
#' }
#' The historical entry is left unchanged.
#'
#' \strong{Methodology note:} Ensemble bounds use month-wise quantiles of
#' perturbed weather values across models. This synthetic series does not
#' preserve cross-month physical coherence. Acceptable for uncertainty band
#' display; not a physically coherent trajectory. Full per-model simulation
#' required for coherent ensemble bounds.
#'
#' @param weather_result Named list as returned by \code{get_weather()}.
#'   Must contain a \code{"historical"} entry. Future entries are expected
#'   to follow the naming convention
#'   \code{<ssp>_<start>_<end>_<model_name>}.
#' @param lo_q  Lower quantile for the ensemble bound. Default \code{0.10}.
#' @param hi_q  Upper quantile for the ensemble bound. Default \code{0.90}.
#'
#' @return A named list with the same structure as \code{weather_result} but
#'   with per-model future entries replaced by three representative entries
#'   per SSP/period group. The \code{"historical"} entry and any
#'   \code{attr()} attributes (e.g. \code{stored_breaks}) are preserved.
#'
#' @export
summarise_ensemble <- function(weather_result, lo_q = 0.10, hi_q = 0.90) {
  ...
  # Parse SSP/period prefix from key: <ssp>_<start>_<end>_<model>
  # Key format: ssp3_7_0_2025_2035_ModelName (5 underscore-separated
  # segments before the model name — ssp + scenario digits + start + end).
  # Regex captures everything up to and including the 5th segment.
  .key_prefix <- function(key) {
    m <- regmatches(key, regexpr("^(?:[^_]+_){4}[^_]+", key))
  ...
  rep_entries <- lapply(unique_prefixes, function(pfx) {
    ...
    # Summarise across models per (loc_id x timestamp x other meta)
    # Using base R for speed — avoids dplyr grouping overhead on small tables
    group_cols <- intersect(meta_cols, names(stacked))
    grp        <- interaction(stacked[group_cols], drop = TRUE)

    mean_df <- stats::aggregate(
      stacked[w_vars],
      by   = stacked[group_cols],
      FUN  = mean,
      na.rm = TRUE
    )

    lo_df <- stats::aggregate(
      stacked[w_vars],
      by   = stacked[group_cols],
      FUN  = function(x) stats::quantile(x, lo_q, na.rm = TRUE)
    )

    hi_df <- stats::aggregate(
      stacked[w_vars],
      by   = stacked[group_cols],
      FUN  = function(x) stats::quantile(x, hi_q, na.rm = TRUE)
    )
  ...
  rep_flat <- do.call(c, rep_entries)

  # Build result: historical unchanged + 3 representatives per SSP/period
  result_new         <- c(list(historical = weather_result[["historical"]]), rep_flat)
  attr(result_new, "stored_breaks") <- attr(weather_result, "stored_breaks")
  attr(result_new, "ensemble_summarised") <- TRUE
  attr(result_new, "ensemble_lo_q")        <- lo_q
  attr(result_new, "ensemble_hi_q")        <- hi_q
  attr(result_new, "n_models_per_group")   <- vapply(
    unique_prefixes,
    function(pfx) sum(prefixes == pfx),
    integer(1L)
  )
