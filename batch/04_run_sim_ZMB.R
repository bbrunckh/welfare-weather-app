...
#
# Outputs (under OUT_DIR/model_fit/, when SAVE_MODEL_FIT = TRUE):
#   model_coefficients.parquet           coefficient estimates (fit1/fit2/fit3)
#   model_fit_stats.parquet              R², AIC, nobs per model
#   _interactions_not_available.csv      skipped specs (interaction var missing)
#
...
# =============================================================================
# SECTION 3 — HELPERS AND PRE-LOOP SETUP
# =============================================================================
...
# ---- Model fit output paths (conditional) ------------------------------------
if (SAVE_MODEL_FIT) {
  dir.create(file.path(OUT_DIR, "model_fit"), showWarnings = FALSE, recursive = TRUE)
  .mf_coef_pq     <- file.path(OUT_DIR, "model_fit", "model_coefficients.parquet")
  .mf_stats_pq    <- file.path(OUT_DIR, "model_fit", "model_fit_stats.parquet")
  .mf_int_na_csv  <- file.path(OUT_DIR, "model_fit", "_interactions_not_available.csv")
  .mf_coef_dedup  <- c("code", "weather", "engine", "fe_profile", "cov_profile",
                        "interaction", "model", "term", "tau")
  .mf_stats_dedup <- c("code", "weather", "engine", "fe_profile", "cov_profile",
...
# ---- OVERWRITE_EXISTING cleanup ------------------------------------------------
# When overwriting, delete all output files upfront then switch to FALSE so
# per-country flushes use merge/append mode.
...
if (OVERWRITE_EXISTING) {
  ...
  if (SAVE_MODEL_FIT) {
    for (.f in c(.mf_coef_pq, .mf_stats_pq, .mf_int_na_csv)) {
      if (file.exists(.f)) file.remove(.f)
    }
  }
...
# =============================================================================
# SECTION 4 — PER-COUNTRY PIPELINE
# =============================================================================
# Process one country fully (fit -> simulate -> policy -> flush to disk -> free
# memory) before moving to the next.  Bounds peak memory to ~one country's
# worth of data regardless of how many countries are in SAMPLE_LABELS.
...
for (si in SAMPLE_LABELS) {
  ...
  cat(sprintf("\n\n========== %s (%d/%d) ==========\n",
              si, match(si, SAMPLE_LABELS), length(SAMPLE_LABELS)))
  ...
  if (SAVE_MODEL_FIT) {
    country_mf_coefs <- list()
    country_mf_stats <- list()
  ...
  if (nrow(ss) == 0) {
    ...
    .save_csv(data.frame(spec_label = si, policy_label = NA_character_,
                          stage = "model_fitting", error = "no_surveys",
                          stringsAsFactors = FALSE),
              out_failures, .fail_keys)
    next
  }
  ...
  if (is.null(svy_base)) {
    ...
    total_failed <- total_failed + 1L
    .save_csv(data.frame(spec_label = si, policy_label = NA_character_,
                          stage = "model_fitting", error = "load_failed",
                          stringsAsFactors = FALSE),
              out_failures, .fail_keys)
    next
  }
  ...
  for (wx_name in wx_profiles) {
    ...
    for (mi in seq_len(nrow(model_combos))) {
      ...
      # ---- Extract model_fit outputs (mirrors 03_run_mod1.R) -------------------
      if (SAVE_MODEL_FIT) {
        ...
        if (!.fit1_key %in% .seen_fit1) {
          .r1 <- do.call(extract_one_fit,
            c(list(fit = mf$fit1, model_label = "fit1", code = si), .fit_args))
          if (!is.null(.r1$coefs))     country_mf_coefs[[length(country_mf_coefs) + 1L]] <- .r1$coefs
          if (!is.null(.r1$fit_stats)) country_mf_stats[[length(country_mf_stats) + 1L]] <- .r1$fit_stats
        ...
        if (!.fit2_key %in% .seen_fit2) {
          .r2 <- do.call(extract_one_fit,
            c(list(fit = mf$fit2, model_label = "fit2", code = si,
                   fe_label = fe_label, fe_vec = fe_vec), .fit_args))
          if (!is.null(.r2$coefs))     country_mf_coefs[[length(country_mf_coefs) + 1L]] <- .r2$coefs
          if (!is.null(.r2$fit_stats)) country_mf_stats[[length(country_mf_stats) + 1L]] <- .r2$fit_stats
          .seen_fit2 <- c(.seen_fit2, .fit2_key)
        }

        # fit3 (full model) — always unique per spec
        .lasso_sel <- if (exists("lasso_res") && !is.null(lasso_res) &&
                          length(lasso_res$selected_covariates) > 0)
          paste(lasso_res$selected_covariates, collapse = "|") else NA_character_
        .r3 <- do.call(extract_one_fit,
          c(list(fit = mf$fit3, model_label = "fit3", code = si,
                 ...
                 lasso_selected_vars = .lasso_sel), .fit_args))
        if (!is.null(.r3$coefs))     country_mf_coefs[[length(country_mf_coefs) + 1L]] <- .r3$coefs
        if (!is.null(.r3$fit_stats)) country_mf_stats[[length(country_mf_stats) + 1L]] <- .r3$fit_stats
      }
  ...
  total_fitted  <- total_fitted + n_fitted
  total_failed  <- total_failed + length(fail_log)
  total_skipped <- total_skipped + length(skip_log)
  cat(sprintf("  Step 1: %d fitted, %d failed, %d skipped\n",
              n_fitted, length(fail_log), length(skip_log)))

  rm(svy_base)
  gc(verbose = FALSE)

  # Seed per-country failure accumulator from fitting failures
  ...
  if (n_fitted == 0L) {
    .save_csv(dplyr::bind_rows(country_fail_rows), out_failures, .fail_keys)
    cat(sprintf("  %s done (no fitted models)\n", si))
    rm(fit_store, fail_log, skip_log, country_fail_rows)
    gc(verbose = FALSE)
    next
  }
  ...
  for (spec_label in names(fit_store)) {
    ...
    cat(sprintf("\n  [%d/%d] %s\n", country_sim_idx, n_fitted, spec_label))
    ...
    for (pol_name in .pols_here) {
      ...
      cat(sprintf("  [%d] %s : %s...", country_pol_idx, spec_label, pol_name))
  ...
  .save_parquet(dplyr::bind_rows(country_yr_decile_rows), out_yr_decile, .yr_decile_keys)
  .save_csv(dplyr::bind_rows(country_fail_rows), out_failures, .fail_keys)

  # Policy config: new rows win on dedup so latest run_status is preserved
  if (length(country_pol_cfg_rows) > 0L) {
    new_cfg <- dplyr::bind_rows(country_pol_cfg_rows)
  ...
  # Model fit outputs (coefficients + fit stats + skip log)
  if (SAVE_MODEL_FIT) {
    if (length(country_mf_coefs) > 0L)
      .save_parquet(clean_names(dplyr::bind_rows(country_mf_coefs)), .mf_coef_pq, .mf_coef_dedup)
    if (length(country_mf_stats) > 0L)
      .save_parquet(clean_names(dplyr::bind_rows(country_mf_stats)), .mf_stats_pq, .mf_stats_dedup)
    if (length(skip_log) > 0L) {
      .new_skip <- data.frame(
      ...
      .save_csv(.new_skip, .mf_int_na_csv, c("spec_label"))
    }
  }
  ...
  if (SAVE_MODEL_FIT) rm(country_mf_coefs, country_mf_stats, .seen_fit1, .seen_fit2)
  gc(verbose = FALSE)
...
cat("\n================================================================\n")
cat(sprintf("Pipeline complete: %d fitted, %d failed, %d skipped, %d simulated, %d policy runs\n",
            total_fitted, total_failed, total_skipped, sim_run_idx, pol_run_idx))
cat("================================================================\n")
