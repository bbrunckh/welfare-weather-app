...
test_that("make_weather_effect_plot returns ggplot for continuous predictor, no interaction", {
  skip_if_not_installed("jtools")
  fit <- make_lm()
  p   <- make_weather_effect_plot(fit, pred_var = "tx")
  expect_s3_class(p, "ggplot")
})
...
test_that("make_weather_effect_plot returns ggplot with interaction (binned)", {
  ...
  fit <- lm(y ~ tx + urban + tx:urban, data = d)
  p   <- make_weather_effect_plot(
    fit               = fit,
    pred_var          = "tx",
    interaction_terms = "tx:urban",
...
test_that("make_weather_effect_plot applies label_fun to axis labels", {
  skip_if_not_installed("jtools")
  fit <- make_lm()
  p   <- make_weather_effect_plot(fit, pred_var = "tx", label_fun = toupper)
  expect_s3_class(p, "ggplot")
...
test_that("make_weather_effect_plot returns blank plot when all pred values non-finite", {
  skip_if_not_installed("jtools")
  set.seed(1)
  d   <- data.frame(y = rnorm(50), tx = rep(NaN, 50))
  fit <- lm(y ~ tx, data = d)
  p   <- make_weather_effect_plot(fit, pred_var = "tx")
  expect_s3_class(p, "ggplot")
  built  <- ggplot2::ggplot_build(p)
  labels <- unlist(lapply(built$data, function(d) d$label))
  expect_true(any(grepl("finite|cannot|not found", labels, ignore.case = TRUE)))
})
...
test_that("plot_pred_vs_actual returns ggplot for logistic model", {
  skip_if_not_installed("ggplot2")
  set.seed(1)
  d   <- data.frame(y = rbinom(100, 1, 0.4), tx = rnorm(100))
  fit <- glm(y ~ tx, data = d, family = binomial)
...
test_that("calc_fit_stats returns 4-row data frame for linear model", {
  fit <- make_lm()
  out <- calc_fit_stats(fit, is_logistic = FALSE)
  expect_s3_class(out, "data.frame")
  expect_equal(nrow(out), 4L)
  expect_setequal(names(out), c("Statistic", "Value"))
})

test_that("calc_fit_stats returns 6-row data frame for logistic model", {
  set.seed(1)
  d   <- data.frame(y = rbinom(100, 1, 0.4), tx = rnorm(100))
  fit <- glm(y ~ tx, data = d, family = binomial)
  out <- calc_fit_stats(fit, is_logistic = TRUE)
