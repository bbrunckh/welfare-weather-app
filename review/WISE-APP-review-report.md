...
> **Remediation status (through 2026-09-01):** Five fix waves applied; completed rows are compressed under their sections, open rows keep full detail. Wave 0 determinism (DET-01..09) complete 2026-08-31, plus two low-risk batches and scoped parts of REACT-01/13, TEST-03/05, RED-09. On 2026-09-01: a blocker batch (DEP-01, INT-02/09, DEP-02/03 incl. the REACT-01 remainder, REACT-05, SEC-02, TEST-06/09 gradient bugs), a test batch (TEST-07 raw-weights contract, TEST-08 packaging), four output-neutral, equivalence-verified perf batches — **Batch A** (PERF-23b/08/25/10), **Batch B** (DUP-01, PERF-30/REACT-07), **Batch C** (PERF-22/34, RNG-stream-invariant, bit-identical), **Batch D** (PERF-32) — a quick-win batch (INT-04, INT-07, UI-04, UI-38, RED-05; REACT-04/06 closed as no-action), and three High-severity fixes (REACT-12 failure ledger + TEST-01 remainder; REACT-02 busy guards + UI-01 visible errors; PERF-31 Step 3 aggregation cache), plus the remaining standalone perf wins PERF-23 (Step 1 h3 temp table), PERF-13 (versioned weather disk cache), and PERF-16 (dev mode removed entirely). Full edition-3 suite: **547/547 pass**; built-source `R CMD check` clean at the code level (remaining findings environment-only — no `pdflatex`); `R CMD build` succeeds. Deployment commits `0b73a4c`/`459d3f7`/`fdd3960` — see §10.1.
