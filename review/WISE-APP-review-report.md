...
> **Remediation status (through 2026-09-01):** Five fix waves applied; completed rows are compressed under their sections, open rows keep full detail. Wave 0 determinism (DET-01..09) complete 2026-08-31, plus two low-risk batches and scoped parts of REACT-01/13, TEST-03/05, RED-09. On 2026-09-01: a blocker batch (DEP-01, INT-02/09, DEP-02/03 incl. the REACT-01 remainder, REACT-05, SEC-02, TEST-06/09 gradient bugs), a test batch (TEST-07 raw-weights contract, TEST-08 packaging), four output-neutral, equivalence-verified perf batches — **Batch A** (PERF-23b/08/25/10), **Batch B** (DUP-01, PERF-30/REACT-07), **Batch C** (PERF-22/34, RNG-stream-invariant, bit-identical), **Batch D** (PERF-32) — a quick-win batch (INT-04, INT-07, UI-04, UI-38, RED-05; REACT-04/06 closed as no-action), and three High-severity fixes (REACT-12 failure ledger + TEST-01 remainder; REACT-02 busy guards + UI-01 visible errors; PERF-31 Step 3 aggregation cache). Full edition-3 suite: **533/533 pass** (module testServer harnesses for mod_2_02/mod_2_03/Step 3 results pane added); built-source `R CMD check` clean at the code level (remaining findings environment-only — no `pdflatex`); `R CMD build` succeeds. Deployment commits `0b73a4c`/`459d3f7`/`fdd3960` — see §10.1.
...
**Done (2026-09-01), compressed (Batches A-D + PERF-31, all output-neutral, equivalence/bit-identical verified):** PERF-08 (single grouped `rowsum()` completeness pass, ~3x at 200k×72) · PERF-10 (parsed GeoJSON copies dropped from feature builders; `.geojson_bounds()` parses string-only features transiently) · PERF-22 (policy deltas + training ecdf precomputed once per run and threaded through the per-(scenario × year) decomposition loop; 1.5x at 50k HH, RNG-free, bit-identical) · PERF-23b (hash-cached `CREATE OR REPLACE SECRET` extended to S3/GCS/Azure) · PERF-25 (grouping hoisted out of the per-variable weather loop via `.summarise_loc_prep(prep =)`) · PERF-30 (display-only `band_q` dropped from `agg_workspace()`; poverty-line/bandwidth folded into method-specific cache keys, so moving those controls no longer destroys unrelated cached methods) · PERF-31 (Step 3 aggregation cache in `fct_policy_sim_compare.R`: fresh cache environment on any sim republish, keyed by (source, method, poverty line) only — `cmp_deviation` is applied downstream (hist_ref subtraction + axis labels via the new `agg_axis_label()`), so moving the deviation control no longer re-aggregates baseline + every scenario member; `.wire_results_pane()` exposes internals for tests; cached ≡ uncached verified bit-identically across the method/deviation/pov-line grid, testServer invalidation matrix) · PERF-32 (prediction frame released before the N×K matrix build; process peak now bounded by the join phase, byte-identical) · PERF-34 (ID→residual lookup + residual variance built once per pipeline/member; bit-identical, RNG unchanged). Regression coverage: `test-mod_2_02_results.R`, `test-policy-sim-compare-agg-cache.R`.
