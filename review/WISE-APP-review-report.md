...
> **Remediation status (through 2026-09-01):** Five fix waves applied. Wave 0 determinism (DET-01..09) is complete, plus two low-risk batches (dead code, dedup, perf hoists, REACT-10, UI-29..31/33..35/39/40, DUP-02, RED-02/03/04/07, SEC-01/04/05, TEST-02/04) and scoped parts of REACT-01/13, TEST-03/05, RED-09. A 2026-09-01 blocker batch fixed DEP-01, INT-02/INT-09, DEP-02/DEP-03 (incl. the REACT-01 remainder), REACT-05, SEC-02, and the TEST-06/TEST-09 gradient bugs. A 2026-09-01 test batch decided the TEST-07 statistical contract (raw weights) and completed TEST-08. Four 2026-09-01 perf batches followed, all output-neutral (equivalence-verified): **Batch A** (PERF-23b secret-SQL caching, PERF-08 grouped completeness, PERF-25 grouping hoist, PERF-10 GeoJSON copies), **Batch B** (DUP-01 single `resolve_band_q`, PERF-30/REACT-07 cache-key narrowing), **Batch C** (PERF-22 deltas/ecdf precompute, PERF-34 residual-lookup cache — RNG-stream-invariant, bit-identical), **Batch D** (PERF-32 prediction-frame release). Verified with `devtools::document()`, `pkgload::load_all()`, and the full edition-3 suite: **all 462 tests pass** (module harnesses for mod_2_02 added); built-source `R CMD check` is clean at the code level (remaining findings are environment-only — no `pdflatex`). `R CMD build` succeeds. Completed rows are struck through and compressed below; open rows keep full detail. Deployment commits `0b73a4c`/`459d3f7`/`fdd3960` — see §10.1.
...
| **PERF-22** | ~~`R/fct_policy_decompose.R:449, 119` recomputes policy deltas and `stats::ecdf()` per scenario-year~~ ✅ **DONE** (2026-09-01): `decompose_policy_effect()`, `.decompose_rif()`, `.compute_rif_channels()`, and `.compute_rif_policy_correction()` accept optional pre-computed `deltas` / `F_hat` (weather- and scenario-independent; NULL = compute internally, fully backward compatible); `apply_policy_delta_to_baseline()` and the `mod_3_06_policy_sim.R` per-(scenario × year) loop now build both once per run and pass them through. RNG-free change (neither piece consumes the stream). Measured 1.5x on 30 successive OLS decompositions at 50k HH (deltas are a smaller share of OLS cost; the RIF path saves the per-call training-ecdf build). Cached ≡ uncached verified bit-identically. | Complete. |
...
| **PERF-34** | ~~Default residual aggregation rebuilds the same ID-to-residual named vector and character keys per year, member, method, and weighting (`R/fct_aggregation.R:215-221`)~~ ✅ **DONE** (2026-09-01): `.residual_lookup()` / `.residual_sigma2()` build the matched-ID lookup and residual variance once per pipeline; passed through `aggregate_with_uncertainty_delta()` (NULL-defaulted, backward compatible) from `aggregate_pipeline_per_year()` (once per member) and `mod_2_02_results.R`'s builders (once per member per build). The normal path uses `sqrt(cached var)` — bit-identical to `sd()`. Unmatched-ID sampling stays in the call path, so RNG consumption is unchanged; cached ≡ uncached verified bit-identically. | Complete. |
